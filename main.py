...
# ОДИН ОБРАБОТЧИК ДЛЯ ВСЕХ НАЖАТИЙ НА СТАДИИ ВЫБОРА (БЕЗОТКАЗНО РАБОТАЕТ)
...
async def tp_target_choice(cb: types.CallbackQuery, state: FSMContext):
    if cb.data == "target_all":
        ...
        await cb.message.edit_text(f"Введите время для <b>{nick}</b> (мин):", parse_mode="HTML")
        await state.set_state(TechPause.entering_time)
    await cb.answer()
