...
async def tp_sel_one(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(f"Время для {n} (мин):"); await state.set_state(TechPause.entering_time); await cb.answer()
...
async def tp_time(m: types.Message, state: FSMContext):
    ...
    await m.answer("Автовыключение?", reply_markup=kb); await state.set_state(TechPause.choosing_auto_off)
...
async def tp_final(cb: types.CallbackQuery, state: FSMContext):
    ...
    msg = await cb.message.edit_text(f"🛠 <b>Техперерыв: {d['target']}</b> (+{d['mins']}м)"); await state.clear(); await cb.answer()
...
async def del_tp_exec(cb: types.CallbackQuery):
    pause_data.pop(cb.data.replace("del_tp_", ""), None); await save_data(); await refresh_panels(); await tp_del_menu(cb)
...
async def cmd_info(m: types.Message):
    ...
    status_messages[cid] = msg.message_id; await save_data()

@dp.callback_query(F.data == "tp_back")
async def tp_back(cb: types.CallbackQuery): await cb.message.edit_text(get_status_text(), reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔄 Настройки", callback_data="ask_reset")]]), parse_mode="HTML"); await cb.answer()
