...
async def ping_cmd(message: types.Message):
    ...
    last_sent_text = "" 
    msg = await bot.send_message(message.chat.id, "<b>🐝 Запуск панели...</b>", parse_mode="HTML")
...
async def add_cmd(message: types.Message, command: CommandObject):
    ...
    await save_to_db(); await message.answer(f"✅ Пинг для <code>{rbx}</code> активен", parse_mode="HTML")
